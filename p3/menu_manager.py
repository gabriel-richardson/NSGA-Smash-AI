...
class MenuManager:
    ...
    # THIS IS TERRIBLE PROGRAMMING BUT I COULDN'T FIGURE OUT ANOTHER WAY AND IT'S NOT RELATED
    # TO WHAT MY CLASS PROJECT IS REALLY ABOUT, SORRY
    def change_settings(self, state, pad):
        if self.changed_settings:
            pad.reset()
            self.press_start_lots(state, pad)
        else:
            if state.frame % 365 == 0:
                pad.press_button(p3.pad.Button.D_LEFT)
            elif state.frame % 370 == 0:
                pad.release_button(p3.pad.Button.D_LEFT)
            elif state.frame % 375 == 0:
                pad.press_button(p3.pad.Button.D_DOWN)
            elif state.frame % 380 == 0:
                pad.release_button(p3.pad.Button.D_DOWN)
            elif state.frame % 385 == 0:
                pad.press_button(p3.pad.Button.D_LEFT)
            elif state.frame % 390 == 0:
                pad.release_button(p3.pad.Button.D_LEFT)
            elif state.frame % 395 == 0:
                pad.press_button(p3.pad.Button.D_LEFT)
            elif state.frame % 400 == 0:
                pad.release_button(p3.pad.Button.D_LEFT)
