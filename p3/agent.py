...
class Agent:
    ...
    def update(self, state, pad):
        ...
        inputs.append(self.normalize(state.players[1].self_air_vel_y, -2.79, 3.22)) # speed y
        ...
        if outputs[3] >= .5: # Up
            self.action_list.append((1, pad.tilt_stick, [p3.pad.Stick.MAIN, 0.5, 1.0]))
        if outputs[4] >= .5: # Top-Left
            self.action_list.append((1, pad.tilt_stick, [p3.pad.Stick.MAIN, 0.0, 1.0]))
        if outputs[5] >= .5: # Top-Right
            self.action_list.append((1, pad.tilt_stick, [p3.pad.Stick.MAIN, 1.0, 1.0]))
        if outputs[6] >= .5: # Bottom-Left
            self.action_list.append((1, pad.tilt_stick, [p3.pad.Stick.MAIN, 0.0, 0.0]))
